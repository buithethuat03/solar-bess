...
> **Version:** 2.0
> **Status:** Draft toàn platform; US-001/operational foundation/US-003 core Implemented; US-004 Implemented local, TEST-014…017 Partial và GitHub Actions/EC2 deployment Pending; US-005 Implemented local với AC-021 Not covered; US-019 Partial với AC-088/090/092 Not covered; US-006/US-007 Implemented local với AC-023…026/028…030 Partial và AC-027/031/032 Not covered; production Proposed
...
| DB-001…114 | 114 | 07 Data Model | DB-069…072, DB-022…027/114 và DB-028…031/034…040 materialized 2026-07-26; DB-114 cấp mới cho external share theo phê duyệt Product Owner; **DB-032/033 cố ý chưa tạo — không operation nào trong catalog ghi chúng, không phải bị quên**; DB-101…105 và DB-112/113 materialized local; DB-106…111 reserved; US-004 forward reconcile đã test local, actual deploy Pending |
| API-001…164 | 164 | 08 API/OpenAPI | x-api-id exact/unique; 88 implemented markers gồm API-008/036/038/039…066/106…112/135/136/143…164, trong đó API-039…052 và API-053…066 materialize ngày 2026-07-26; mỗi marker có controller thật; direct/dependency ownership vẫn tách rõ |
...
| Enterprise control | BR-003, BR-009, BR-011, BR-012, BR-019, BR-026, BR-035 | FR-026…FR-035 | UC-005 | WF-004…006 | US-005 | AC-018…AC-022 | API-039…052 | DB-022…027; DB-114 external share; DB-098 audit; DB-102/104 | SEC-109/112/113/118…123/126 | TEST-018…TEST-022 | MVP; API-039…052 và DB-022…027/114 Implemented local; AC-018/019/020/022 đóng; AC-021/TEST-021 (full-text/OCR) Not covered — không có search index/OCR và không operation nào trong catalog 164; UI Vue `/projects/:projectId/documents` đã có (register, bộ lọc, phân trang cursor, upload hai bước, ba verdict tách bạch), Playwright chưa có kịch bản |
| Enterprise control | BR-009…BR-011, BR-022, BR-026, BR-030 | FR-036…FR-044 | UC-006 | WF-008/009 | US-006 | AC-023…AC-027 | API-053…061; API-039…052 | DB-028…033; DB-022…027 | SEC-108/109/114/118/119/126/130; SEC-109/112/113/118…123/126 | TEST-023…TEST-027 | MVP; API-053…061 và DB-028…031 Implemented local 2026-07-26 (DB-032/033 cố ý chưa tạo — không operation nào ghi chúng); AC-023/024/025/026 Partial — snapshot party băm bằng canonicalHash tại thời điểm thêm party nhưng "tại thời điểm ký" bất khả thi vì không operation nào ký/kích hoạt hợp đồng (DRAFT-only với đủ từ vựng 8 trạng thái), không phụ lục nào đạt EFFECTIVE, guarantee/cảnh báo mốc vắng, từ chối sửa văn bản ký chứng minh bằng trigger ở tầng DB; AC-027/TEST-027 Not covered — không condition/COD gate; UI Vue `/projects/:projectId/contracts` đã có |
| Enterprise control | BR-007, BR-015, BR-030, BR-033 | FR-053…FR-060, FR-138…FR-155 | UC-007 | WF-014 | US-007 | AC-028…AC-032 | API-062…066; API-053…061 | DB-034…040; DB-028…033 | SEC-108/109/114/118/119; SEC-108/109/114/118/119/126/130 | TEST-028…TEST-032 | MVP; API-062…066 và DB-034…040 Implemented local 2026-07-26; AC-028/029/030 Partial — commitment chống trùng theo source version, payment một-transaction với constraint trigger deferred SUM(components) = requested_amount (422 `COMPONENT_SUM_MISMATCH` rollback trọn slice), số gốc theo currency + fx_snapshots bất biến, không cộng chéo currency kể cả UI, quy đổi hiển thị cố ý vắng (NFR-013 TBD); AC-031/032 và TEST-031/032 Not covered — không operation approve budget/payment (ck_payment_sod/ck_budget_version_sod cài trước) và không EAC/contingency/alerting; tiền không bao giờ tính trong JS — numeric(19,4) là string, tổng tính trong SQL |
...
| Enterprise control | BR-011, BR-035, BR-040 | FR-029…FR-035, FR-145, FR-151…FR-155, FR-164 | UC-019 | WF-007 | US-019 | AC-088…AC-092 | **Direct:** API-051/052 trong API-039…052<br>**Dependency:** API-001…014; API-106…113 | DB-114; DB-027; DB-022…027; DB-001…008/098; DB-069…072 | SEC-109/112/113/118…123/126; SEC-101…110/118; SEC-102/106…110/118 | TEST-088…TEST-092 | MVP; API-051/052 Implemented local; AC-089/TEST-089 Partial (policy share có bằng chứng, không operation nào phục vụ byte nên preview/download/redemption chưa có); AC-091/TEST-091 Partial (chặn sửa nội dung ISSUED có bằng chứng, chưa envelope nào COMPLETED); AC-088/090/092 và TEST-088/090/092 Not covered — approval theo classification + MFA, revoke/expiry enforcement và e-sign callback đều chưa được contract |
...
| ADR-005; US-005/US-019; SEC-120/121/126 | Object storage + malware gate cho document control | DB-022…027/114; API-039…052; MinIO + ClamAV qua hai port `object-storage.port.ts`/`malware-scanner.port.ts` | TEST-018…020/022 Pass; TEST-089/091 Partial; TEST-021/088/090/092 Not covered; `ck_document_revision_release_requires_clean` chặn phát hành khi chưa `CLEAN` | Implemented local, chưa deploy; adapter-live suite chỉ chạy nơi có container `minio-test`/`clamav-test` | ADR-005 Accepted cho EC2 test profile; production Proposed — retention/lifecycle bucket, KMS at-rest, HA MinIO và cập nhật signature tự động vẫn TBD |
...
| API-001…164 | Each x-related-requirements/data/security | OpenAPI x-api-id + API/contract/security tests | 164/164 have requirement; 88 current controller-backed operations (bổ sung API-039…052 document control và API-053…066 contract & cost ngày 2026-07-26) and 76 remain design-only; acceptance status remains test-specific |
| DB-001…114 | Domain/BR/FR/NFR trace in dictionary/operational plan | API/OpenAPI, aggregate/internal relationship or explicit reservation | 114/114 referenced outside owner; DB-022…027, DB-114 và DB-028…031/034…040 materialized local ngày 2026-07-26 (DB-032/033 cố ý chưa tạo — không operation nào ghi chúng); DB-069…072 materialized; DB-101…105 and DB-112/113 materialized local; DB-106…111 reserved; forward reconcile tested local, actual deploy Pending |
...
- DB-022…027/114 và API-039…052 đã materialize nhưng **không** khép kín US-005/US-019: AC-021 full-text/OCR không có search index/OCR pipeline và không operation nào trong catalog 164 phục vụ nó; AC-089 chỉ ghi được policy vì không operation nào phục vụ byte nên watermark/preview/download và redemption link chưa tồn tại; AC-088 approval theo classification + xác minh recipient/MFA và AC-092 e-sign callback chưa được contract. Các cột `status`/`revoked_*` của DB-114 và trigger envelope COMPLETED là chỗ chờ sẵn, không phải feature đã chạy.
...
| 1.9 | 2026-07-26 | Codex | Ghi implementation trace document control US-005/US-019: API-039…052, DB-022…027, DB-114 cấp mới, ADR-005 Accepted cho EC2 test profile; reconcile registry lên 114 DB entity và 74 controller-backed operation | Không đổi requirement/AC; cấp mới đúng một DB ID. AC-018/019/020/022 đóng; AC-089/091 Partial; AC-021/088/090/092 Not covered và được ghi rõ ở backlog, changelog lẫn matrix. Hàng 1.7/1.8 chưa từng được ghi vào bảng này cho slice US-022/US-015; không tái tạo lịch sử, chỉ ghi nhận khoảng trống |
| 2.0 | 2026-07-26 | Codex | Ghi implementation trace contract & cost US-006/US-007: API-053…066, DB-028…031/034…040 materialized, DB-032/033 cố ý chưa tạo vì không operation nào ghi chúng; reconcile registry lên 88 controller-backed operation | Không đổi requirement/AC; không cấp ID mới. AC-023…026/028…030 Partial, AC-027/031/032 Not covered — điểm dừng có chủ ý tại chỗ catalog không cấp operation (không gì ký/kích hoạt hợp đồng, không approve budget/payment, không condition/COD gate, không EAC/alerting); SoD CHECK cài trước ở tầng DB. Spec-defect ghi làm follow-up đính chính: off-by-one FR-039/040/041 giữa docs/03 và docs/08/docs/07, x-idempotency ghi DB-101 thay vì DB-104, dictionary DB-038 thiếu invoice_id |
