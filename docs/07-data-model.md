...
> **Purpose:** Định nghĩa mô hình dữ liệu logic, ERD, 112 entity/store ID, khóa, constraint, index, tenant isolation, lifecycle, classification và retention cho PM Web, O&M, operational foundation và read-only telemetry.
> **Scope:** Logical/implementation-ready data contract; PostgreSQL operational stores được chấp nhận cho EC2 test. US-003 concretizes DB-012/017…021/101 và schedule subset của DB-105; US-004 concretizes DB-065…067/112 nhưng chưa có migration/runtime evidence; DB-068 Claim/Variation giữ dependency Contract/Legal. Production DDL/partition/HA vẫn qua ADR/benchmark.
...
> **Version:** 0.8
> **Status:** Draft toàn platform; US-001 và US-003 core data Implemented/deployed EC2 test; US-004 DB-065…067/112 Approved/Build-ready, chưa Implemented; DB-068 dependency Reserved
> **Owner:** Data Architecture / Domain Owners (cá nhân: TBD)
> **Updated:** 2026-07-12
> **Approval:** Product Owner delegated approval cho US-003 EC2 test contract và US-004 EC2 test data contract; US-004 migration/test/deploy chưa thực hiện, production vẫn Proposed
...
|---|---|---|---|
| Transactional relational | DB-001…090, DB-093…097, DB-099…112 | Aggregate transaction; tenant/project indexes; PITR | DB-101…105 materialized cho EC2 test; DB-106…111 Reserved; DB-112 Approved/Build-ready, chưa materialized |
...
erDiagram
 ...
 DB_010_PROJECT ||--o{ DB_101_PROJECT_SCHEDULE : reserves_for_US003
 DB_001_TENANT ||--o{ DB_105_NOTIFICATION : reserves
...
`DB-102…104` thuộc operational foundation đã materialize. `DB-101` và phần schedule-alert của `DB-105` đã có migration/table trong US-003; phần Risk/Issue notification của `DB-105` và `DB-112` mới ở trạng thái Approved/Build-ready. `DB-106…111` tiếp tục Reserved cho slice sau; `DB-068` là dependency Contract/Legal, không thuộc migration US-004.
...
| <a id="db-005"></a> **DB-005 — UserAccount** | Organization & Access | id PK; tenantId; personId FK? | issuer; externalSubject; email; status; lastLoginAt | UQ tenant+issuer+externalSubject; IDX status/email | Mutable; Confidential; disable preserves history | BR-033/040; FR-146…155 |
...
| <a id="db-065"></a> **DB-065 — Risk** | Risk/Change | id PK; tenantId+projectId composite FK; packageId? composite FK to DB-012 cùng project; ownerId/closureRequesterId/closureDecidedBy composite FKs to DB-005 | code; category; cause; event; impact; probability + cost/schedule/HSE impact ratings integer 1…5; derived impactRating/inherentExposure; responseStrategy; responsePlan; trigger; contingencyPlan; residualProbability/residualImpact/residualExposure; reviewDate; evidenceRefs; status; closure evidence/reason/actors/times; versionNo; actor/timestamps | UQ tenant+project+code; UQ tenant+project+id; IDX tenant+project+package/status/owner/residualExposure/reviewDate; server computes impactRating=max(cost,schedule,HSE), exposure=probability×impactRating; residual score uses residual inputs; OCCURRED requires linked same-scope DB-066 | Approved/Build-ready, chưa materialized; mutable/versioned in active states; CLOSED/OCCURRED facts and closure decision protected; Confidential; retention project lifecycle/legal hold | BR-022/031/032; FR-098/099/104; US-004; AC-014/015/017; TEST-014/015/017 |
| <a id="db-066"></a> **DB-066 — Issue** | Risk/Change | id PK; tenantId+projectId composite FK; packageId? composite FK to DB-012; sourceRiskId? composite FK to DB-065 cùng project/package scope; ownerId/closureRequesterId/closureDecidedBy composite FKs to DB-005 | code; title; description; severity LOW/MEDIUM/HIGH/CRITICAL; occurredAt; actualImpact; rootCause; decisionSummary; ownerId; targetDate; evidenceRefs; sourceRiskId?; status; resolution/closure evidence/reason/actors/times; versionNo; actor/timestamps | UQ tenant+project+code; UQ tenant+project+id; IDX tenant+project+package/status/severity/owner/targetDate; source Risk phải cùng tenant/project và Issue kế thừa packageId; transition source Risk → OCCURRED và link Issue là một transaction | Approved/Build-ready, chưa materialized; mutable/versioned đến closure; VERIFIED/CLOSED decision/evidence protected; reopen tạo audited transition, không xóa lịch sử; Confidential | BR-022/031/032; FR-100/104; US-004; AC-014/015/017; TEST-014/015/017 |
| <a id="db-067"></a> **DB-067 — ChangeRequest** | Risk/Change | id PK; tenantId+projectId composite FK; packageId? composite FK to DB-012; sourceRiskId?/sourceIssueId? same-scope composite FK; sourceBaselineId? composite FK to DB-020; requesterId/submitterId/decidedBy composite FKs to DB-005 | code; sourceType MANUAL/RISK/ISSUE; source refs; sourceEvidenceSnapshot; sourceBaselineId; reason; options; recommendation; scopeImpact; scheduleImpactDays; scheduleImpactSummary; costImpactAmount numeric(19,4)? + ISO currency?; qualityImpact; hseImpact; contractImpact; impactSnapshot/hash; approvalSnapshot/hash; decisionVersion; status; submitted/decision actors/times/reason; versionNo; actor/timestamps | UQ tenant+project+code; UQ tenant+project+id; IDX tenant+project+package/status/requester/submittedAt; sourceType/ref XOR; source-derived Change kế thừa packageId; amount/currency both null or both present; six impact dimensions complete trước SUBMITTED; requester/submitter ≠ decision actor; only APPROVED same-project record whose sourceBaselineId matches current baseline and whose approved schedule impact is explicit may authorize DB-020 REBASELINE | Approved/Build-ready, chưa materialized; Draft/Assessed/Returned versioned; submitted decision append-audited; approved source/impact/decision/approval snapshots and hashes immutable, no hard delete; Confidential | BR-018/022/031/032; FR-101/102/104; US-003/004; AC-012/016; TEST-012/016 |
...
| <a id="db-105"></a> **DB-105 — Notification** | Notification | id PK; tenantId+recipientUserId composite FK; projectId/activityId logical scoped refs | sourceType; sourceId; alertType; priority; objectLink; reason; dueAt; dataDate; thresholdVersion; dedupKey; status; readAt; createdAt | UQ tenant+dedupKey; IDX tenant+recipient+status/dueAt; notification never grants access; recipient scope rechecked before projection/delivery | Schedule subset Implemented EC2 test under US-003/AC-013; Risk/Issue action overdue and decision-queue projection Approved/Build-ready nhưng chưa materialized; full preference/digest/channel lifecycle remains US-022 | BR-022/032/034/038; FR-020/025/099/100/104/142…145/175/177; US-003/004/022; TEST-013/015/103…107 |
...
| <a id="db-112"></a> **DB-112 — RiskIssueAction** | Risk/Change | id PK; tenantId+projectId composite FK; packageId? composite FK to DB-012; riskId?/issueId? same-scope composite FKs; ownerId/completedBy/verifiedBy composite FKs to DB-005 | code; actionType RESPONSE/CONTINGENCY/CORRECTIVE/DECISION; description; ownerId; dueDate; status OPEN/IN_PROGRESS/BLOCKED/DONE/VERIFIED/CANCELLED; evidenceRefs; residualProbability?/residualImpact?; completedAt/by; verifiedAt/by; versionNo; actor/timestamps | UQ tenant+project+code; exactly one of riskId/issueId; packageId kế thừa parent; IDX tenant+project+package+owner+status+dueDate and parent+status; residual inputs 1…5 and only valid for Risk action; DONE/VERIFIED requires evidence | Approved/Build-ready, chưa materialized; mutable/versioned; completion/verification retained; Confidential; no hard delete after history/outbox reference | BR-022/031/032; FR-099/100/104; US-004; AC-015/017; TEST-015/017 |
...
- Direct aggregate là `DB-065 Risk`, `DB-066 Issue`, `DB-067 ChangeRequest` và `DB-112 RiskIssueAction`. `DB-068 Claim/Variation` phụ thuộc physical Contract/Legal aggregate của US-006; US-004 không tạo table, route hoặc trạng thái giả cho Claim.
- Risk dùng thang integer 1…5; server tính `impactRating = max(costImpactRating, scheduleImpactRating, hseImpactRating)`, `inherentExposure = probability × impactRating`, `residualExposure = residualProbability × residualImpact`. EC2 V1 phân loại HIGH từ 15 và CRITICAL từ 20; derived score không nhận từ client; threshold lấy từ env có range/cross-field validation và ghi threshold version vào projection, không sửa score lịch sử khi cấu hình đổi.
- Risk states: `IDENTIFIED → ASSESSED → TREATING → MONITORING → CLOSURE_PENDING → CLOSED`; active state có thể sang `OCCURRED` chỉ khi cùng transaction tạo/link Issue cùng tenant/project/package scope. Closure bị reject quay về trạng thái active trước request được ghi trong `DB-098`; không đóng Risk bằng comment hoặc khi còn DB-112 bắt buộc chưa `VERIFIED`.
- Issue states: `REPORTED → TRIAGED → IN_PROGRESS → RESOLVED → CLOSURE_PENDING → VERIFIED → CLOSED`; reject closure quay về `RESOLVED`; closure chỉ được approve khi action bắt buộc đã `VERIFIED`; explicit reopen ghi `REOPENED → IN_PROGRESS` cùng evidence/audit, không xóa decision cũ.
- Closure decision actor phải khác creator, current owner và closure requester; Change decision actor phải khác requester/submitter. Constraint identity được re-check trong transaction kể cả khi actor có nhiều role hoặc delegation; quyền `closeCritical` cho HIGH/CRITICAL là policy input, không phải cột tự cấp quyền.
...
- Public application contract `APPROVED_CHANGE_READER.resolveForRebaseline(manager, { tenantId, projectId, changeRequestId, currentBaselineId })` resolve trong transaction caller và không cho Project Controls import entity/repository Risk/Change. Success trả approvedAt/by, decisionVersion, sourceBaselineId, scheduleImpactSummary và impactSnapshotHash từ approval snapshot bất biến. Stable denial là `NOT_FOUND_OR_SCOPE_MISMATCH`, `NOT_APPROVED`, `BASELINE_MISMATCH` hoặc `SCHEDULE_IMPACT_NOT_APPROVED`; mọi denial tạo zero baseline side effect.
- Migration expand-first: tạo candidate keys/DB-065…067/112, kiểm orphan/cross-scope, thêm composite FK DB-020 → DB-067 rồi validate; không tự sửa scope. Trigger bảo vệ approved Change impact/decision/snapshot/hash và approved baseline provenance khỏi UPDATE/DELETE. Down chỉ dùng disposable/zero-row database; khi đã có approval/rebaseline phải disable route/worker và forward-fix, không drop source/audit.
...
| PostgreSQL 17 là physical store cho DB-001…090/093…112 trong EC2 test; DB-101…105 đã materialize đúng slice; DB-065…067/112 mới là contract US-004 | Product Owner delegated 2026-07-11/12 | Không suy rộng contract thành migration/runtime evidence hoặc production HA/partition/SoR |
...
| **Closed for US-004 EC2 contract:** DB-065/066/067/112 fields, state, composite scope, score và immutable approval snapshot; migration/runtime vẫn chưa thực hiện | Product Owner delegated 2026-07-12 | M0 docs unblocked; không claim implementation/test pass |
