...
|---|---|---|---|
| Transactional relational | DB-001…090, DB-093…097, DB-099…119 | Aggregate transaction; tenant/project indexes; PITR | DB-106/107 materialized 2026-07-26; DB-108…111 vẫn Reserved (chưa duyệt provider/policy AI); DB-115…119 cấp mới 2026-07-26 theo ủy quyền Product Owner; DB-083 và DB-089…092/096/097 cố ý chưa tạo — không operation nào trong catalog ghi chúng |
...
| <a id="db-106"></a> **DB-106 — SavedView** | Reporting | id PK; tenantId; ownerUserId composite FK | name; targetType; filterSnapshot; columnSnapshot; sortSnapshot; shareScope; versionNo | UQ tenant+owner+target+name; IDX tenant+owner/target | Materialized 2026-07-26 qua `1783755000000`; V1 chỉ cho `shareScope = PRIVATE` (CHECK một giá trị) nên một view đã lưu về cấu trúc không thể nâng quyền cho ai; Confidential | BR-001/032/036/038; FR-171…177; US-023; TEST-108…112 |
| <a id="db-107"></a> **DB-107 — ReportJob** | Reporting | id PK; tenantId; requestedBy composite FK | reportType; filterSnapshot; dataAsOf; status; outputObjectRef; expiresAt; correlationId | UQ tenant+id; IDX tenant+requester+status/createdAt; idempotency via DB-104 | Materialized 2026-07-26 qua `1783755000000`; output snapshot bất biến khi COMPLETED (trigger); V1 trả tham chiếu object **không** phải presigned URL — package presigner chưa được cài, khoảng trống có ghi nhận | BR-001/032/036/038; FR-130…137/171…177; US-023; TEST-108…112/178 |
...
| <a id="db-114"></a> **DB-114 — DocumentExternalShare** | Document Control | id PK; tenantId; revisionId composite FK to DB-023 | tokenHash; recipientSnapshot; purpose; expiresAt; watermarkRequired; downloadAllowed; downloadCount; status ACTIVE/REVOKED/EXPIRED; revokedBy/At; createdBy/At | UQ tenant+id; UQ tokenHash; IDX revision+status; IDX tenant+expiresAt; CHECK expiresAt > createdAt; CHECK revoked pair; CHECK downloadCount >= 0 | Chỉ chia sẻ được revision đã ISSUED. **Chỉ lưu SHA-256 của token**, plaintext trả về đúng một lần lúc tạo nên rò rỉ database không sinh ra link dùng được. Mặc định watermarkRequired = true, downloadAllowed = false. Confidential/legal-hold aware | BR-011/035; FR-029/033/145/164; US-019; AC-089/090; API-051; SEC-119…123; TEST-089/090 |
| <a id="db-115"></a> **DB-115 — StopWorkAction** | Field/HSE | id PK; tenantId+projectId composite FK; siteId?/workfrontId?/permitToWorkId?/hseIncidentId? composite FKs; liftsActionId? self-FK; actorId composite FK to DB-005 | action ISSUE/LIFT; targetType PROJECT/SITE/WORKFRONT/PERMIT; reason; verifiedControls; evidenceRefs; authorityRoleCode; occurredAt; correlationId | CHECK target shape khớp targetType; CHECK LIFT phải trỏ liftsActionId; CHECK LIFT cần ≥1 verifiedControls; partial UQ một LIFT cho mỗi ISSUE; trigger BEFORE INSERT từ chối lift bởi chính người ban; trigger append-only chặn mọi UPDATE/DELETE | **Ledger append-only, không phải cờ trạng thái**: "công việc có đang bị dừng không" là một truy vấn, không phải một ô ai đó lật được. Cấp mới 2026-07-26 theo ủy quyền Product Owner, cùng tiền lệ DB-114 | BR-019/026; FR-088/089; US-011; AC-051; API-094; SEC-108/109/118; TEST-051 |
| <a id="db-116"></a> **DB-116 — NcrDispositionCycle** | Quality | id PK; tenantId+projectId composite FK; ncrId composite FK to DB-060; proposedBy/decidedBy composite FKs to DB-005 | sequenceNo; proposedDisposition; proposalRationale; proposalEvidenceRefs; decision APPROVE/RETURN/REJECT; decisionComment; decisionEvidenceRefs; resultingStatus | UQ tenant+ncr+sequenceNo; partial UQ đúng một cycle chưa quyết định; CHECK cặp decision đồng nhất; CHECK decidedBy ≠ proposedBy; trigger đóng băng cycle đã quyết định | Reopen nối tiếp `sequenceNo`, không ghi đè: mở lại một NCR không được xóa bằng chứng của lần thẩm định trước. Cấp mới 2026-07-26 theo ủy quyền Product Owner, mô phỏng DB-113 | BR-026; FR-093; US-010; AC-047; API-096; SEC-108/118; TEST-047 |
| <a id="db-117"></a> **DB-117 — PunchClosureCycle** | Quality | id PK; tenantId+projectId composite FK; punchItemId composite FK to DB-061; requestedBy/decidedBy composite FKs to DB-005 | sequenceNo; requestComment; requestEvidenceRefs; decision APPROVE/RETURN; decisionComment; decisionEvidenceRefs; resultingStatus | UQ tenant+punch+sequenceNo; partial UQ đúng một cycle chưa quyết định; CHECK cặp decision đồng nhất; CHECK decidedBy ≠ requestedBy; trigger đóng băng cycle đã quyết định | Cùng lý do DB-116 cho vòng đóng punch (WF-018 cho phép Closed → Reopened). Cấp mới 2026-07-26 theo ủy quyền Product Owner | BR-026; FR-094; US-010; AC-046; API-097; SEC-108/118; TEST-046 |
| <a id="db-118"></a> **DB-118 — CodGateReviewCycle** | Commissioning/COD | id PK; tenantId+projectId composite FK; codGateId composite FK to DB-076; submittedBy/decidedBy composite FKs to DB-005 | sequenceNo; submissionComment; submissionEvidenceRefs; decision ACCEPT/RETURN/WAIVE; decisionComment; decisionEvidenceRefs; resultingStatus | UQ tenant+gate+sequenceNo; partial UQ đúng một cycle chưa quyết định; CHECK cặp decision đồng nhất; CHECK decidedBy ≠ submittedBy; trigger đóng băng cycle đã quyết định | AC-059 đòi lịch sử từng vòng nộp/duyệt kèm người và bằng chứng; DB-076 chỉ giữ trạng thái hiện tại. Cấp mới 2026-07-26 theo ủy quyền Product Owner, mô phỏng DB-116 | BR-023/030; FR-110…112; US-013; AC-058/059/060; API-105; SEC-108/118; TEST-058…060 |
| <a id="db-119"></a> **DB-119 — WorkOrderClosureCycle** | O&M | id PK; tenantId+projectId composite FK; workOrderId composite FK to DB-086; requestedBy/decidedBy composite FKs to DB-005 | sequenceNo; requestComment; requestEvidenceRefs; decision APPROVE/RETURN; decisionComment; decisionEvidenceRefs; resultingStatus | UQ tenant+workOrder+sequenceNo; partial UQ đúng một cycle chưa quyết định; CHECK cặp decision đồng nhất; CHECK decidedBy ≠ requestedBy; trigger đóng băng cycle đã quyết định | WF-024 cho phép VERIFIED → REOPENED; không có bảng chu trình thì một lần mở lại sẽ ghi đè `verified_by`/`verified_at` và xóa mất quyết định thẩm định trước. Cấp mới 2026-07-26 theo ủy quyền Product Owner | BR-030/032; FR-118…121; US-014; AC-065; API-120; SEC-108/118; TEST-065 |
