...
- **Delivery status:** **Implemented (AC-018/019/020/022), AC-021 Not covered** ngày 2026-07-26. Mười bốn operation `API-039…052` và bảy bảng `DB-022…027` + `DB-114` đã materialize trong module Nest `document-control`, kèm route Vue `/projects/:projectId/documents` gated theo `document.read`. AC-018 đóng bằng chuỗi quarantine → hash SHA-256 do server tính trên byte đã lưu → verdict ClamAV: `CLEAN` promote sang release bucket rồi phát hành, `INFECTED` hủy byte quarantine và mọi lệnh downstream trả 422 `REVISION_NOT_SCANNED`, `UNAVAILABLE` ghi bền vững, giữ byte và trả 503 `SCANNER_UNAVAILABLE` để retry sau vẫn thành công; adapter chỉ trả `CLEAN` cho đúng reply khớp `/^stream:\s*OK$/i` nên sự cố scanner làm chậm phát hành chứ không âm thầm phát hành file chưa quét. AC-019 đóng bằng `CHECK ck_document_revision_release_requires_clean` — `APPROVED/ISSUED/SUPERSEDED` không tồn tại được nếu thiếu `scan_status = 'CLEAN'`, `content_hash` và `released_object_key` — cộng trigger `trg_document_revision_issued_immutable` (vẫn cho ISSUED → SUPERSEDED) và issue chuyển `current_revision_id` cùng supersede bản cũ trong một transaction. AC-020 đóng bằng transmittal chỉ nhận revision đã ISSUED, snapshot đúng revision và ghi recipient/action/due date/response; chưa có delivery receipt qua kênh gửi ngoài và chưa có job đặt `response_status = OVERDUE`. AC-022 đóng cho các liên kết đã tồn tại (register → current revision → history, lọc theo project/package/type/discipline/classification, cắt scope ngay trong SQL); liên kết tới contract/equipment/vendor chưa có vì `DB-028…033`/`DB-041…050` chưa được triển khai. **AC-021 (full-text/OCR search) Not covered** — không có search index, không có OCR pipeline và không operation nào trong catalog 164 phục vụ việc này; đây là điểm dừng có chủ ý. Mọi khóa ngoại composite mang `tenant_id`; đọc ngoài phạm vi trả 404 chứ không 403. UI hiển thị scan status là điểm nhấn: revision chưa `CLEAN` không được cấp nút hành động nào thay vì mời bấm một nút mà server chắc chắn từ chối, và register nhúng cả revision đã ISSUED lẫn revision mới nhất nên một upload `INFECTED` chưa từng phát hành vẫn hiện ra đúng chỗ người vận hành nhìn. Evidence: integration HTTP 19 + migration 12 = 31 Pass cộng 7 test adapter-live nói giao thức thật với MinIO/clamd (gồm chuỗi EICAR), toàn bộ integration API Pass không hồi quy, unit API và web Pass, lint/typecheck/openapi:lint Pass.
...
- **Delivery status:** **Partial — AC-023/024/025/026 Partial, AC-027 Not covered** ngày 2026-07-26. `API-053…061` đã materialize trong module Nest `contract-cost` (cùng slice với `API-062…066` của US-007), kèm 4 bảng `DB-028…031` và route Vue `/projects/:projectId/contracts` gated theo `contract.read`. **AC-023 Partial** — số hợp đồng duy nhất trong dự án (`uq_contract_project_number`, trùng trả 409 zero-write), các bên dùng ID pháp nhân ổn định và snapshot được băm bằng `canonicalHash` tách từ risk-change (byte-identical, không có serializer thứ hai) tại thời điểm **thêm party**; snapshot "tại thời điểm ký" bất khả thi vì không operation nào trong catalog 164 ký/kích hoạt hợp đồng — hợp đồng ship DRAFT-only với CHECK mang đủ từ vựng 8 trạng thái để không cần migration sau. Party snapshot chỉ giữ đúng những gì master `legal_entities` thật có (legal_name, country, registration_no, tax_id) cộng representative/authority do client khai; không có address vì master không có. **AC-024 Partial** — chuỗi phụ lục theo `(appendix_no, revision_no)` và giá trị hợp nhất là read model SQL từ chuỗi văn bản có hiệu lực; nhưng không phụ lục nào đạt được EFFECTIVE vì không operation nào chuyển trạng thái nó, và tiền đề "cha effective" của API-058 được nới có ghi nhận thành "cha tồn tại và không CLOSED/EXPIRED" (deviation tường minh khỏi WF-008). **AC-025 Partial** — obligation đủ owner/beneficiary/due/evidence/consequence, CHECK bắt evidence khi FULFILLED/WAIVED nên upload tệp không tự đóng gì, chip DUE/OVERDUE suy diễn trong projection SQL từ `due_date` chứ không lưu; guarantee vắng (`DB-032` cố ý không tạo — không operation nào ghi nó) và cảnh báo theo mốc vắng (allowlist notification đóng). **AC-026 Partial** — từ chối sửa văn bản đã ký enforce bằng trigger `trg_contract_history` (họ signed đóng băng danh tính + legal hold + cấm thoái lui), chứng minh ở tầng DB bằng test migration UPDATE tay; end-to-end qua API cần một hợp đồng ký được — chưa tồn tại. **AC-027 Not covered** — không engine đánh giá condition, không COD gate, không operation waiver; điểm dừng có chủ ý. SoD obligation (decider ≠ owner ≠ creator) enforce bằng CHECK tầng hàng. Mọi FK composite mang `tenant_id`; FK mang currency `(tenant_id, id, currency)` làm `CURRENCY_MISMATCH` bất khả thi về cấu trúc; ngoài scope trả 404 chứ không 403. Evidence: nằm trong integration contract-cost 16 HTTP + 13 migration Pass (API tổng 163/17 suite), unit/lint/typecheck/openapi:lint Pass, 88/164 marker implemented.
...
- **Delivery status:** **Partial — AC-028/029/030 Partial, AC-031/AC-032 Not covered** ngày 2026-07-26. `API-062…066` đã materialize trong module Nest `contract-cost` (cùng slice với `API-053…061` của US-006), kèm 7 bảng `DB-034…040` và UI cost summary + form budget/commitment/payment trong route `/projects/:projectId/contracts`. Server không làm phép tính tiền nào trong JS: `numeric(19,4)` là string qua TypeORM, mọi tổng và đối chiếu — giá trị hợp nhất, tổng thành phần payment — tính bằng Postgres numeric. **AC-028 Partial** — commitment chống trùng bằng `uq_commitment_source (tenant, source_type, source_id, source_version)` nên replay là 409 chứ không phải hàng đôi, ledger append-only, drill-down qua cost summary và API-066; baseline approved và forecast/EAC vắng vì không operation approve/cost engine nào tồn tại (partial unique index "một budget APPROVED mỗi dự án" và trigger đóng băng APPROVED đã cài trước). **AC-029 Partial** — payment một-transaction đủ trường bắt buộc (contract, payer/payee bằng pháp nhân, currency, gross/VAT/deduction qua components, milestone, evidence) cộng constraint trigger `DEFERRABLE INITIALLY DEFERRED` khẳng định SUM(components) = requested_amount; service ép chạy trong transaction bằng `SET CONSTRAINTS ALL IMMEDIATE` nên breakdown sai là 422 `COMPONENT_SUM_MISMATCH` có tên và cả slice payment+components+invoice rollback cùng nhau; invoice chống trùng `(tenant, contract, supplier_legal_entity, invoice_no)`; `payments.invoice_id` được thêm dạng nullable composite FK dù dictionary DB-038 không liệt kê (amendment ghi nhận) để giữ truy vết; không có bước submit và kiểm lũy kế so văn bản hiệu lực chưa có ý nghĩa khi không văn bản nào effective được — tiền đề API-065 nới có ghi nhận thành "cha tồn tại và không CLOSED/EXPIRED" (deviation khỏi WF-009). **AC-030 Partial** — số gốc giữ nguyên theo currency, `fx_snapshots` bất biến và unique theo source/date/pair, không chỗ nào cộng chéo currency kể cả UI; hiển thị quy đổi cố ý vắng vì chưa có quy tắc làm tròn (`NFR-013` TBD). **AC-031 Not covered** — không bước phê duyệt nào tồn tại để loại PM ra khỏi; `ck_payment_sod` (approver ≠ requester/submitter) và `ck_budget_version_sod` (approver ≠ submitter) cài trước ở tầng DB cho lúc operation approve ra đời. **AC-032 Not covered** — không EAC, không contingency threshold, không alerting. Cost code là master data seeded idempotent vì catalog không có operation CRUD nào cho `DB-034` (spec gap có owner); `commitments.purchase_order_id` cố ý vắng cho tới khi Procurement materialize `DB-049`; invoice không có CHECK `net = gross − vat` vì quy tắc thuế TBD. Payments không bao giờ xóa được, PAID đóng băng trừ đối soát; components bất biến. Evidence: nằm trong integration contract-cost 16 HTTP + 13 migration Pass (API tổng 163/17 suite), mọi nhánh 4xx assert zero-write, xuyên tenant/package-only trả 404; lint/typecheck/openapi:lint/build Pass (chunk `ProjectContractsView` 65.6 kB), 88/164 marker implemented.
...
- **Delivery status:** **Nửa engineering Implemented local (AC-033 Partial); nửa procurement đang triển khai riêng** ngày 2026-07-26. Phần engineering của story — BOM và equipment model — đã materialize trong module Nest `engineering-plants`: `API-067/068` (catalog equipment model theo tenant) và `API-069/070` (register + release BOM theo dự án) chạy trên `DB-041` equipment_models, `DB-042` bill_of_materials, `DB-043` bom_lines. **AC-033 Partial — nửa engineering đóng được:** tiền đề "BOM/requisition đã duyệt" nay có thật — `API-070` release BOM trong một transaction, đòi đúng một design revision ISSUED + scan-CLEAN được re-check bên trong command transaction (422 `DESIGN_REVISION_LOCKED` zero-write) và băm các line đã ghi thành `snapshot_hash` bằng `canonicalHash` dùng chung (không serializer thứ hai), nên Procurement có một revision/specification cố định để RFQ tham chiếu; SoD enforce là releaser ≠ người upload design revision (`RELEASE_SOD` — deviation có ghi nhận vì create-and-release làm "releaser ≠ tác giả BOM" bất khả thi); một BOM RELEASED mỗi dự án (partial unique), nội dung đã release bất biến bằng trigger kể cả chặn INSERT smuggle line. Phần sourcing/RFQ/vendor của AC-033 và `AC-034…AC-037` thuộc nửa procurement (`API-076…085`, `DB-044…054`) đang triển khai riêng — chưa claim gì ở đây. **`API-071` substitutions DEFERRED:** bảng của nó không có DB ID được cấp trong dictionary và không ID nào bị bịa. Migration `1783744000000` đồng thời cấp hai candidate key dùng chung `uq_sites_tenant_project_id`/`uq_wbs_nodes_tenant_project_id` mở khóa FK cho Field/HSE + Procurement. Evidence: nằm trong cổng integration 4 suite / 31 test Pass (engineering migration 9 + seed 4 + risk-change 7 + engineering HTTP 11), unit API 137/23 suite, lint 0 warning, typecheck Pass; marker OpenAPI 88→96 (agent song song trong cùng slice).
...
- **Delivery status:** **Engine Implemented (AC-068…AC-071), AC-072 Not covered** ngày 2026-07-26. `API-106…112` và `DB-069…072` đã materialize kèm Vue `/approvals`. AC-068 đóng bằng validator thuần kiểm tra graph trước publish; AC-069 đóng bằng `route_snapshot` đóng băng lúc start cộng partial unique index chặn hai instance sống trên cùng object; AC-070 đóng bằng ledger append-only có comment bắt buộc, RETURN mở `attempt_no` mới và unique `(instance, step, attempt, actor)`; AC-071 đóng bằng maker-checker BR-034 ở cả service lẫn CHECK constraint và trigger cấm sửa version đã publish. **AC-072 (SLA/nhắc việc/escalation) Not covered** — cột `sla_due_at` và index đã có nhưng luôn NULL vì quy tắc calendar/timezone/pause chưa được trả lời; `API-113` escalate và `workflow.escalate` chưa được cấp. `CONDITIONAL_APPROVE` nằm trong từ vựng DB nhưng bị service từ chối bằng `CONDITIONAL_APPROVE_NOT_ENABLED` cho tới khi có danh sách control không thể miễn trừ. Delegation (`effective_actor_id`) đã có cột nhưng luôn bằng `actor_id` cho tới US-018. Engine chạy độc lập, chưa nối vào `API-154…156` của US-004. Evidence: integration API 18 + migration 9, unit routing 13, Web unit 11, lint/typecheck/OpenAPI/build Pass.
...
- **Delivery status:** **Partial — AC-089/AC-091 Partial, AC-088/AC-090/AC-092 Not covered** ngày 2026-07-26. `API-051` (external share) và `API-052` (signature envelope) đã materialize cùng slice `API-039…052`, kèm bảng `DB-114` cấp mới cho external share và `DB-027` cho signature envelope. **AC-089 Partial** — share khóa vào đúng một revision đã ISSUED, mặc định `watermarkRequired = true` và `downloadAllowed = false`, token plaintext trả về đúng một lần và chỉ SHA-256 của nó được lưu nên rò rỉ database không sinh ra link dùng được, token không bao giờ xuất hiện trong response hay payload audit; nhưng **không operation nào trong catalog phục vụ byte**, nên render watermark, preview/download và redemption link chưa tồn tại. **AC-091 Partial** — phần chặn sửa nội dung đã ISSUED có `trg_document_revision_issued_immutable` cộng test HTTP/migration, và review ledger append-only bằng trigger riêng; phần "signature và timestamp giữ nguyên" chưa kiểm chứng được vì chưa envelope nào đạt COMPLETED. **AC-088 Not covered** — không có tuyến approval theo classification và không xác minh recipient/MFA; share chỉ do quyền `documentShare.create` quyết định và `SEC-102` vẫn không khả thi trong base auth profile. **AC-090 Not covered** — không có redemption để chặn và không có operation revoke; cột `status`/`revoked_by`/`revoked_at` và `CHECK expires_at > created_at` đã sẵn nhưng hiện chỉ được test migration dùng. **AC-092 Not covered** — `API-052` tạo envelope DRAFT cục bộ với `artifact_hash` = content hash của revision và không liên hệ provider nào vì chưa provider nào được contract; `ck_signature_envelope_external` giữ `external_id` bất khả thi khi còn DRAFT và `trg_signature_envelope_completed_immutable` đã sẵn cho lúc có callback. `API-051`/`API-052` là create-only trong catalog — không operation nào đọc lại share hay envelope, nên UI không dựng được register cho chúng và slice Vue không phủ phần này. Evidence: nằm trong 31 test document control (19 HTTP + 12 migration) Pass, lint/typecheck Pass.
...
- **Delivery status:** **Phần equipment/asset registry + plant configuration Implemented local (AC-122 Partial); AC-123/124/125 Not covered** ngày 2026-07-26. `API-072/073` đã materialize trong module Nest `engineering-plants` cùng slice với nửa engineering của US-008, trên `DB-079` equipment, `DB-080` assets, `DB-081` solar_plants (kèm `DB-041…043` model/BOM). **AC-122 Partial** — vế "Approved BOM/design có model và quantity" chạy: BOM line trỏ model catalog, quantity `numeric(19,4)` không bao giờ chạm JS number, nội dung release bất biến bằng trigger; đối chiếu BOM↔PO↔receipt chờ nửa procurement của US-008 merge (`DB-044…050` chưa có), và deviation tạo hold/change/NCR cần substitutions (`API-071` DEFERRED — bảng không có DB ID được cấp) + NCR domain. `API-073` release cấu hình theo version từng site: luôn release MAX(site)+1 thành hàng mới và supersede bản RELEASED hiện hành trong cùng transaction, anchor đã SUPERSEDED bị từ chối, một bản RELEASED mỗi site bằng partial unique, lịch sử version bất biến. **AC-123 Not covered** (handover dossier/warranty/as-built/WO thuộc domain handover + O&M chưa có operation), **AC-124 Not covered** (KPI PR/yield/availability cần telemetry read-only từ hệ thống ngoài), **AC-125 Not covered** (không rule engine/alert; không gì trong PM Web chạm inverter/SCADA setpoint — `solar_plants` không có cột credential-shaped, enforce bằng unit guard + assertion `information_schema` theo SEC-127/128). Plant bootstrap bằng seed (quyết định có ghi nhận — không operation nào trong catalog tạo plant); không có UI Vue trong slice này. Evidence: nằm trong cổng integration 4 suite / 31 test Pass, unit API 137/23 suite, lint/typecheck Pass; marker OpenAPI 88→96 (agent song song trong cùng slice).
...
- **Delivery status:** **Phần asset hierarchy registry Implemented local (AC-126 Partial — phần lớn đóng được); AC-127…AC-130 Not covered** ngày 2026-07-26. `API-074/075` đã materialize trong module Nest `engineering-plants` cùng slice với US-008 (nửa engineering)/US-026, trên `DB-082` bess_plants kèm `DB-079` equipment và `DB-080` assets. **AC-126 Partial** — parent-child kiểm bằng FK composite `fk_equipment_parent` mang `tenant_id`; serial duy nhất bằng partial unique serial-claim `uq_equipment_serial` (một equipment sống mỗi serial; chưa FK vì bảng serial thuộc Procurement chưa materialize); replacement giữ lineage cũ–mới qua `replaced_by_id`; một asset sống mỗi equipment (`uq_asset_equipment_active`); equipment không bao giờ DELETE được, RETIRED là terminal, asset archive thay vì xóa; model/effective date có; **trường firmware không có trong dictionary `DB-079/080/082` — TBD có owner, không tự thêm cột.** `API-074` trả hierarchy version + operating envelope và trả KPI/telemetry là `null` tường minh thay vì bịa; `API-075` simulate dispatch là **advisory và stateless**: đánh giá envelope đã RELEASED bằng exact-decimal string, persist duy nhất command receipt và cố ý không phát audit/outbox (test assert đếm trước–sau bằng nhau) — trên demo trả 422 `OPERATING_ENVELOPE_MISSING` cho tới khi catalog cấp operation release BESS envelope. **`bess_plants` không có cột credential/connectivity nào (host/username/password/token/url/endpoint/secret) — SEC-127/SEC-128 là sự thật schema, canh bằng unit guard + assertion `information_schema`;** PM Web không thể chứa nổi một đường tới BESS/PCS/BMS/EMS. **AC-127…AC-130 Not covered** — telemetry/time-series, detection/alarm/call tree, capacity/RTE test và degradation/SOH đều thuộc commissioning/O&M/OT chưa có operation trong catalog; điểm dừng có chủ ý, đúng ranh giới miền. Evidence: nằm trong cổng integration 4 suite / 31 test Pass, unit API 137/23 suite (gồm credential guard + dispatch evaluation + exact-decimal), lint/typecheck Pass; marker OpenAPI 88→96 (agent song song trong cùng slice).
