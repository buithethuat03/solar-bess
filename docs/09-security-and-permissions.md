...
> **Version:** 0.8
> **Status:** Draft toàn platform; US-001 và core US-003 permission/SoD Implemented EC2 test; US-004 permission/SoD contract Approved/Build-ready, chưa Implemented; full security runtime evidence pending
> **Owner:** Product Security / IAM / OT Security (cá nhân: TBD)
> **Updated:** 2026-07-12
> **Approval:** Product Owner delegated approval cho US-003 implementation và US-004 EC2 test permission contract; US-004 runtime/security acceptance chưa thực hiện
...
| `riskChange.manage` | Cập nhật Risk/Issue/Action/Change editable trong project | Chỉ record cùng package; Action và source-derived Issue/Change phải kế thừa package | Không đổi package để nâng scope; không sửa approved/closed immutable fields |
...
| `riskChange.close` | Approve/return closure khi independent | Denied | Actor khác creator, current owner và closure requester; mọi action bắt buộc phải `VERIFIED` |
...
`PROJECT_MANAGER`/`PMO` có full-project assignment mới có thể nhận submit/approve/close primitives theo fixed EC2 role policy; `PROJECT_CONTROLS`/`PACKAGE_OWNER` với package-only assignment chỉ có create/read/manage/requestClosure trong package được gán. `EXECUTIVE` chỉ read; `TENANT_ADMIN` không mặc nhiên có business permission. Production authority matrix, quorum, financial threshold và step-up vẫn là TBD, không được suy từ fixed EC2 grants.

Rebaseline chỉ gọi public application port `APPROVED_CHANGE_READER.resolveForRebaseline` trong transaction Project Controls với `tenantId`, `projectId`, `changeRequestId`, `currentBaselineId`; Project Controls không import Risk/Change entity/repository. Port chỉ trả immutable approvedAt/by, decisionVersion, sourceBaselineId, scheduleImpactSummary và impactSnapshotHash khi đúng scope/state/baseline/approved schedule impact; còn lại dùng `NOT_FOUND_OR_SCOPE_MISMATCH`, `NOT_APPROVED`, `BASELINE_MISMATCH` hoặc `SCHEDULE_IMPACT_NOT_APPROVED`, audit denial vào `DB-098` và tạo zero baseline side effect. Client lấy lịch sử/current baseline qua `API-159` thuộc Project Controls với `schedule.read`; RiskChange không mở private baseline repository/query để né authorization boundary.
...
| Risk/Issue Owner | Risk/Issue/Action | Update assigned item/action, evidence, residual/actual impact; request closure | Decide own closure; submit/approve Change unless separately full-project authorized | Exact assigned project/package record and current ownership |
| Change/Closure Approver | Risk/Change | APPROVE/RETURN/REJECT Change; approve/return closure; critical close only with `closeCritical` | Creator/owner/requester/submitter self-decision; missing verified action/evidence; package-only approval | Full-project assignment + explicit primitive + effective authority |
...
| PACKAGE_OWNER | Schedule/Progress/Risk | Read schedule; record progress; create/read/manage Risk/Issue/Action and request closure for assigned package | Project-level/null or other package record; Change submit/approve; closure decision; dependency/calendar/baseline change | Assigned package/activity and effective company/owner relation |
...
US-003 core controls đã materialize và có unit/static evidence: service re-check tenant/project/package/owner, self-approval denial, immutable approved DB-020, append-only DB-021 và authorized export audit. PostgreSQL known-ID isolation, permission revoke trước notification drill-down và same-scope DB-067 positive rebaseline đã được thiết kế/viết test nhưng final runtime rerun còn Pending; không suy diễn thành full security acceptance.

US-004 controls trong mục 5.1 là canonical contract Approved/Build-ready, chưa có migration, permission seed, integration/E2E hoặc deployment evidence. Không được đọc trạng thái tài liệu này như security Pass; DB-068 Claim/Variation và legal privilege workflow vẫn là dependency của Contract/Legal slice.
...
| **Closed for US-004 EC2 contract:** tám `riskChange.*` primitives, exact package visibility, independent Change/closure decision và `closeCritical` gate | Product Owner delegated / Security | M0/M1 unblocked; runtime evidence vẫn pending |
