...
> **Version:** 1.5
> **Status:** Draft toàn platform; 51 current runtime API operations được đánh dấu Implemented và tách khỏi full 164-operation design contract; TEST-014…017 acceptance Partial và production vẫn Pending
...
| <a id="api-135"></a> **API-135** | GET /v1/notifications | Liệt kê DB-105 notification projection của chính người gọi; meta mang unread counter độc lập filter cho badge | notification.read; recipient là người gọi, project/package scope tái thẩm định mỗi request | safe; opaque cursor, limit 50/100, filter status/priority/sourceType/alertType/projectId | FR-175 / DB-105 / SEC-107, SEC-118 |
| <a id="api-136"></a> **API-136** | POST /v1/notifications/{notificationId}:acknowledge | Đánh dấu READ; body rỗng; recipient khác/package ngoài scope/tenant khác đều 404; acknowledge lần hai là no-op không sinh thêm audit/outbox; không ghi vào object nguồn | notification.acknowledge; recipient là người gọi, project/package scope tái thẩm định mỗi request | required | FR-175 / DB-098, DB-105 / SEC-107, SEC-118 |
...
[openapi.yaml](./openapi/openapi.yaml) version 0.9.2 declares OpenAPI 3.1.0, JSON Schema 2020-12 dialect, all 164 unique x-api-id/operationId values, security schemes, common parameters/responses/schemas and the API-126 webhook. Exactly 53 controller-backed operations — API-001, API-004…008, API-014…025, API-034…038, API-135/136 and API-137…164 — carry `x-implementation-status: implemented`. API-036 includes both INITIAL and approved-Change-backed positive REBASELINE locally. Other unapproved domain schemas may remain GenericCommand/TBD. Marker status is implementation evidence, not a blanket `TEST-014…017` acceptance claim.

Khi `SWAGGER_ENABLED=true`, API runtime fail-fast load canonical file một lần rồi dẫn xuất hai view: `/api/docs/` và `/api/docs/openapi.yaml` chỉ chứa 51 operation có marker implemented; `/api/design-docs/` và `/api/design-docs/openapi.yaml` giữ toàn bộ 164 API thiết kế, gồm API-126 webhook. Runtime không gọi `SwaggerModule.createDocument` và không tạo contract độc lập từ decorator. Bốn route publication là technical surface của `NFR-024`/`TEST-197`, không phải business operation mới nên không cấp `API-*`.
...
| 1.5 | 2026-07-18 | Codex | Tách current Swagger 51 implemented operations khỏi design Swagger đủ 164 API; backfill 18 marker auth/system/project đã triển khai trong OpenAPI 0.9.1 | Không thêm operation/schema/business scope; sửa metadata trạng thái theo controller runtime và ngăn current docs hiển thị API chưa có |
