...
> **Purpose:** Định nghĩa conventions, authentication/authorization, tenant context, error/idempotency/file/webhook/audit/versioning và danh mục 158 API operations, đồng bộ với OpenAPI 3.1.
...
> **Version:** 0.8
> **Status:** Draft toàn platform; US-001 APIs Implemented; API-023/024/034…037/140…142 US-003 core Implemented/deployed; API-038/143…158 US-004 contract Approved/Build-ready, implementation pending
> **Owner:** API Architecture / Domain Teams (cá nhân: TBD)
> **Updated:** 2026-07-12
> **Approval:** Product Owner delegated approval cho US-003/US-004 EC2 test contract; US-004 fixed policy không thay thế production authority matrix TBD
...
- US-003 operations API-023/024/034…037/140…142 use concrete package/calendar/WBS/activity/dependency/baseline/progress/export schemas; `GenericCommand`/untyped `Envelope` are prohibited for these operations. API-038/DB-067 remain a rebaseline dependency, not direct schedule implementation.
- US-004 operations API-038/143…158 use separate concrete Risk, Issue, RiskIssueAction and ChangeRequest schemas. `GenericCommand`/untyped `Envelope` are prohibited; all mutations use DB-104 idempotency, tenant/project re-check, optimistic version where applicable and immutable DB-005/DB-102 facts.
...
| <a id="api-008"></a> **API-008** | GET /v1/users | Liệt kê user/person đã lọc dữ liệu nhạy cảm | user.read; tenant/department | safe | FR-146 / DB-004, DB-005 / SEC-107, SEC-130 |
...
| <a id="api-036"></a> **API-036** | POST /v1/projects/{projectId}/schedule-baselines | Submit INITIAL/REBASELINE immutable snapshot candidate | baseline.submit; project | DB-104 required; expectedScheduleVersion | FR-018 / DB-020, DB-067 dependency, DB-098, DB-102, DB-104 / SEC-108, SEC-109, SEC-111 |
...
| <a id="api-038"></a> **API-038** | POST /v1/projects/{projectId}/risks | Tạo Risk cause–event–impact, probability/impact 1…5 và owner/due | riskChange.manage; full project, package-only denied | DB-104 required | FR-098/099 / DB-005/065/102/104 / SEC-105/107/111/118 |
...
| <a id="api-143"></a> **API-143** | GET /v1/projects/{projectId}/risks | Liệt kê Risk bằng opaque cursor và allowlisted status/owner/exposure/due filters | riskChange.read; full project, field masking | safe; cursor/limit/filter | FR-098/099 / DB-065/112 / SEC-105/107/111/114 |
| <a id="api-144"></a> **API-144** | PATCH /v1/projects/{projectId}/risks/{riskId} | Cập nhật Risk/state/residual assessment; OCCURRED bắt buộc Issue cùng project; closure chỉ chuyển CLOSURE_PENDING | riskChange.manage; full project | DB-104 required + expectedVersion | FR-098/099/104 / DB-005/065/066/102/104 / SEC-108/109/111/118 |
| <a id="api-145"></a> **API-145** | POST /v1/projects/{projectId}/issues | Tạo Issue actual-impact/root-cause riêng; optional source Risk cùng project | riskChange.manage; full project | DB-104 required | FR-100/104 / DB-005/065/066/102/104 / SEC-105/111/118 |
| <a id="api-146"></a> **API-146** | GET /v1/projects/{projectId}/issues | Liệt kê Issue bằng opaque cursor và status/owner/severity/target/source filters | riskChange.read; full project, field masking | safe; cursor/limit/filter | FR-100 / DB-066/112 / SEC-105/107/111/114 |
| <a id="api-147"></a> **API-147** | PATCH /v1/projects/{projectId}/issues/{issueId} | Cập nhật Issue/state/actual impact; closure chỉ chuyển CLOSURE_PENDING | riskChange.manage; full project | DB-104 required + expectedVersion | FR-100/104 / DB-005/066/102/104 / SEC-108/109/111/118 |
| <a id="api-148"></a> **API-148** | POST /v1/projects/{projectId}/risk-issue-actions | Tạo action gắn đúng một Risk hoặc Issue cùng project | riskChange.manage; full project | DB-104 required | FR-099/100 / DB-005/065/066/102/104/112 / SEC-105/111/118 |
| <a id="api-149"></a> **API-149** | PATCH /v1/projects/{projectId}/risk-issue-actions/{actionId} | Cập nhật owner/due/status/evidence và optional residual Risk assessment | riskChange.manage; full project/action owner | DB-104 required + expectedVersion | FR-099/100 / DB-005/065/066/102/104/112 / SEC-109/111/118 |
| <a id="api-150"></a> **API-150** | POST /v1/projects/{projectId}/change-requests | Tạo Change Request manual hoặc từ đúng một Risk/Issue; copy source/evidence snapshot | riskChange.manage; full project | DB-104 required | FR-101/102/104 / DB-005/065…067/102/104 / SEC-105/108/111/118 |
| <a id="api-151"></a> **API-151** | GET /v1/projects/{projectId}/change-requests | Liệt kê Change bằng cursor và status/owner/source filters | riskChange.read; full project, field masking | safe; cursor/limit/filter | FR-101/102 / DB-067 / SEC-105/107/111/114 |
| <a id="api-152"></a> **API-152** | PATCH /v1/projects/{projectId}/change-requests/{changeRequestId} | Cập nhật Draft/Assessed/Returned Change và six-dimension impact draft | riskChange.manage; full project | DB-104 required + expectedVersion | FR-101/102 / DB-005/067/102/104 / SEC-109/111/114/118 |
| <a id="api-153"></a> **API-153** | POST /v1/projects/{projectId}/change-requests/{changeRequestId}:submit | Chốt complete scope/schedule/cost/quality/HSE/contract impact và submit | riskChange.manage; full project | DB-104 required + expectedVersion | FR-101/102 / DB-005/067/102/104 / SEC-108/109/111/118 |
| <a id="api-154"></a> **API-154** | POST /v1/projects/{projectId}/risks/{riskId}:closure-decision | APPROVE/RETURN/REJECT closure với evidence và approver độc lập | riskChange.close; full project | DB-104 required + expectedVersion + SoD | FR-098/099/104 / DB-005/065/102/104 / SEC-108/109/118 |
| <a id="api-155"></a> **API-155** | POST /v1/projects/{projectId}/issues/{issueId}:closure-decision | APPROVE/RETURN/REJECT closure với evidence và approver độc lập | riskChange.close; full project | DB-104 required + expectedVersion + SoD | FR-100/104 / DB-005/066/102/104 / SEC-108/109/118 |
| <a id="api-156"></a> **API-156** | POST /v1/projects/{projectId}/change-requests/{changeRequestId}:decision | APPROVE/RETURN/REJECT submitted Change; requester/submitter không tự duyệt | riskChange.approve; full project | DB-104 required + expectedVersion + SoD | FR-101/102 / DB-005/020/067/102/104 / SEC-108/109/111/118 |
| <a id="api-157"></a> **API-157** | GET /v1/projects/{projectId}/risk-change-summary | Đọc Command Center summary: top exposure, critical issues, overdue actions và pending decisions | riskChange.read; full project/authorized fields | safe; optional owner filter | FR-098…102/104 / DB-065…067/105/112 / SEC-105/107/111/114 |
| <a id="api-158"></a> **API-158** | GET /v1/projects/{projectId}/risk-change-history | Đọc history đã redact bằng cursor và source/event filters | riskChange.read; source object full-project scope | safe; cursor/limit/filter | FR-099…102/104 / DB-005/065…067/112 / SEC-105/107/114/118/119 |
...
| `SubmitScheduleBaselineRequest` | INITIAL/REBASELINE, dataDate, reason, impactSummary, approvedChangeRequestId for REBASELINE, expectedScheduleVersion |
...
| `BaselineDecisionRequest` | decision APPROVE/RETURN/REJECT, required comment for RETURN/REJECT, expectedVersion; server resolves actor and enforces self/delegated SoD |
...
| Schema | Required contract |
|---|---|
| `CreateRiskRequest` / `UpdateRiskRequest` | Risk-only cause/event/impact, probability and impactRating integer 1…5, owner/due, response/trigger/contingency/evidence; server calculates exposure. Update carries `expectedVersion`; `OCCURRED` requires same-project `occurredIssueId`; closure request requires evidence and reason. |
| `CreateIssueRequest` / `UpdateIssueRequest` | Issue-only title, rootCause, actualImpact, severity, owner/target/evidence and optional same-project sourceRiskId; no Risk probability fields. Update carries `expectedVersion`; closure request requires evidence and reason. |
| `CreateRiskIssueActionRequest` / `UpdateRiskIssueActionRequest` | Exactly one of riskId/issueId; owner, due, status, evidence. Update carries `expectedVersion`; completion requires evidence; optional residual assessment applies only to a Risk action. |
| `CreateChangeRequestRequest` / `UpdateChangeRequestRequest` | Source is MANUAL or exactly one Risk/Issue; server copies source/evidence snapshot. Impact draft has separate scope, schedule, cost, quality, HSE and contract dimensions; money is decimal string plus ISO currency. |
| `SubmitChangeRequest` / `ChangeDecisionRequest` | Both carry `expectedVersion`; submit requires all six impact dimensions complete. Decision is APPROVE/RETURN/REJECT; RETURN/REJECT requires comment; requester/submitter cannot decide. Approved impact/decision is immutable and may satisfy same-scope rebaseline only. |
...
| `RiskListResponse`, `IssueListResponse`, `ChangeRequestListResponse`, `RiskChangeHistoryResponse` | Concrete arrays with opaque UUID cursor and bounded limit; allowlisted filters only, stable createdAt/id ordering, server-side tenant/project/field filtering. |
| `RiskChangeSummaryResponse` | Authorized aggregate plus bounded top Risk, critical Issue, overdue Action and pending Change queues; no cross-package elevation or hidden-field inference. |

US-004 stable error codes: `RISK_NOT_FOUND`, `ISSUE_NOT_FOUND`, `CHANGE_REQUEST_NOT_FOUND`, `ACTION_NOT_FOUND`, `INVALID_STATE_TRANSITION`, `IMPACT_INCOMPLETE`, `CLOSE_EVIDENCE_REQUIRED`, `CLOSE_APPROVAL_SOD`, `CHANGE_APPROVAL_SOD`, `CHANGE_APPROVAL_REQUIRED`, `VERSION_CONFLICT`, `PROJECT_SCOPE_DENIED`, `IDEMPOTENCY_KEY_REQUIRED`, `IDEMPOTENCY_CONFLICT`.

Direct US-004 implementation trace is API-038/143…158 with DB-065…067/112 and operational DB-005/102…105. DB-068 Claim/Variation and direct early-warning adapters in FR-103/105 remain explicit downstream dependencies; these APIs contain no Claim command and no O&M/OT write path.
...
[openapi.yaml](./openapi/openapi.yaml) declares openapi 3.1.0, JSON Schema 2020-12 dialect, all 158 unique x-api-id/operationId values, security schemes, common parameters/responses/schemas and the API-126 webhook. API-023/024/034…037/140…142 are concrete and carry `x-implementation-status: implemented`; API-038/143…158 are concrete and `planned`; other unapproved domain schemas may remain GenericCommand/TBD.
