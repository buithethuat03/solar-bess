...
<template>
  <AppLayout>
    <section class="page-heading"><div><p class="eyebrow eyebrow--accent">PROJECT MASTER</p><h1>{{ project?.name ?? 'Chi tiết dự án' }}</h1><p v-if="project" class="lead">{{ project.code }} · {{ project.type }} · {{ portfolioName }}</p></div><div class="page-heading__actions"><el-button v-if="auth.can('riskChange.read')" type="primary" @click="router.push({ name: RouteName.projectRiskChange, params: { projectId }, query: { tab: 'risks' } })">Risk &amp; Change</el-button><el-button v-if="auth.can('schedule.read')" @click="router.push({ name: RouteName.projectSchedule, params: { projectId } })">WBS &amp; Schedule</el-button><el-button v-if="auth.can('document.read')" @click="router.push({ name: RouteName.projectDocuments, params: { projectId } })">Tài liệu</el-button><el-button v-if="auth.can('contract.read')" @click="router.push({ name: RouteName.projectContracts, params: { projectId } })">Hợp đồng</el-button><el-button v-if="auth.can('workfront.read')" @click="router.push({ name: RouteName.projectFieldOperations, params: { projectId } })">Thi công &amp; HSE</el-button><el-button v-if="auth.can('workfront.read')" @click="router.push({ name: RouteName.projectQuality, params: { projectId } })">Chất lượng</el-button><el-button v-if="auth.can('supplier.read')" @click="router.push({ name: RouteName.projectProcurement, params: { projectId } })">Mua sắm</el-button><el-button v-if="auth.can('commissioning.read')" @click="router.push({ name: RouteName.projectCommissioning, params: { projectId } })">Nghiệm thu &amp; COD</el-button><el-button @click="router.push({ name: RouteName.projects })">Danh sách</el-button></div></section>
