...
<template>
  <section class="risk-change-detail" aria-labelledby="change-panel-title">
    ...
    <form v-if="!change || mode === 'EDIT'" class="risk-change-form" @submit.prevent="save">
      <label>Package<select v-model="form.packageId" aria-label="Package" :disabled="Boolean(change)"><option value="" :disabled="!fullProject">Project-level</option><option v-for="item in packageOptions" :key="item.id" :value="item.id">{{ item.code }} · {{ item.name }}</option></select></label>
      ...
      <label>Source<select v-model="form.sourceType" aria-label="Source" :disabled="Boolean(change) || Boolean(sourceSeed)"><option value="MANUAL">MANUAL</option><option value="RISK">RISK</option><option value="ISSUE">ISSUE</option></select></label>
      ...
      <label v-if="change">Draft state<select v-model="form.status" aria-label="Draft state"><option value="DRAFT">DRAFT</option><option value="ASSESSED">ASSESSED</option></select></label>
