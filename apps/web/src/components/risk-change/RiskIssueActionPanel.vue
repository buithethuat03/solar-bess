...
<template>
  <section class="risk-change-detail" aria-labelledby="action-panel-title">
    ...
    <form v-if="canManage && !terminal && !awaitingIndependentDecision && mode === 'EDIT'" class="risk-change-form" @submit.prevent="submitEdit">
      ...
      <label>Action type<select v-model="form.actionType" aria-label="Action type" :disabled="Boolean(action)"><option v-for="item in ['RESPONSE','CONTINGENCY','CORRECTIVE','DECISION']" :key="item" :value="item">{{ item }}</option></select></label>
      ...
      <label v-if="action">Status<select v-model="form.status" aria-label="Status"><option value="OPEN">OPEN</option><option value="IN_PROGRESS">IN_PROGRESS</option><option value="BLOCKED">BLOCKED</option></select></label>
    ...
    <form v-else-if="canManage && action && !awaitingIndependentDecision && mode === 'COMPLETE'" class="risk-change-form" @submit.prevent="complete">
      ...
      <fieldset v-if="parent.type === 'RISK'" class="form-wide residual-panel"><legend><label><input v-model="residualEnabled" type="checkbox" /> Kèm versioned residual proposal</label></legend><div v-if="residualEnabled" class="risk-change-form"><label>Probability<select v-model.number="form.residualProbability" aria-label="Residual probability"><option v-for="value in 5" :key="value" :value="value">{{ value }}</option></select></label><label>Cost<select v-model.number="form.residualCost" aria-label="Residual cost"><option v-for="value in 5" :key="value" :value="value">{{ value }}</option></select></label><label>Schedule<select v-model.number="form.residualSchedule" aria-label="Residual schedule"><option v-for="value in 5" :key="value" :value="value">{{ value }}</option></select></label><label>HSE<select v-model.number="form.residualHse" aria-label="Residual HSE"><option v-for="value in 5" :key="value" :value="value">{{ value }}</option></select></label><label class="form-wide">Rationale<textarea v-model.trim="form.residualRationale" rows="2" /></label></div></fieldset>
