...
const stateTable = 'role_grant_reconcile_1783749000000';
const policyVersion = 10;

/**
 * Procurement & Logistics permission codes (API-076…API-085; API-079 deferred, so no `bid.submit`
 * is granted — nothing is granted "for later").
 *
 * PMO and PROJECT_MANAGER carry the full procurement authority; PROJECT_CONTROLS records
 * requisitions, evaluations and logistics facts but neither issues RFQs/POs nor submits awards;
 * PACKAGE_OWNER may record goods receipts for its packages' deliveries and nothing else;
 * EXECUTIVE and TENANT_ADMIN get the read-only supplier register (a tenant-level master catalog,
 * unlike the financial data that stays denied to TENANT_ADMIN per docs/09).
 */
...
export class GrantProcurementPermissions1783749000000 implements MigrationInterface {
  name = 'GrantProcurementPermissions1783749000000';
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE TABLE IF NOT EXISTS ${stateTable} (
      ...
      CONSTRAINT ck_role_grant_procurement_permissions CHECK
