...
export class DocumentControlService {
  ...
  async listDocuments(
    ...
    if (cursor) {
      // Same row-wise keyset as `applyCursor`, spelled out because the register is raw SQL: the
      // cursor ISO string only carries milliseconds, so the boundary has to be read back from
      // `documents` or rows whose `created_at` has sub-millisecond digits fall between the branches.
      const at = `${bind(cursor.createdAt)}::timestamptz`;
      const id = `${bind(cursor.id)}::uuid`;
      predicates.push(
        `((document.created_at, document.id) < (
            SELECT boundary.created_at, boundary.id FROM documents boundary WHERE boundary.id = ${id}
          ) OR (
            NOT EXISTS (SELECT 1 FROM documents missing WHERE missing.id = ${id})
            AND (document.created_at < ${at}
              OR (document.created_at = ${at} AND document.id < ${id}))
          ))`
      );
    }
  ...
  async getDocument(context: RequestContext, documentId: string, query: DocumentDetailQueryDto) {
    ...
    // The revision history pages the child table, so the boundary row lives in `document_revisions`.
    if (cursor) this.applyCursor(builder, 'revision', cursor, 'document_revisions', context.tenantId);
   ...
   * That bites hardest here: a batch of revisions written in one transaction shares one `now()`.
