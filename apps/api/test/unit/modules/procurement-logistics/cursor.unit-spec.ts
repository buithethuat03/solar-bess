...
import {
  decodeProcurementCursor, encodeProcurementCursor
} from 'src/modules/procurement-logistics/domain/cursor';

describe('Procurement keyset cursor — API-076', () => {
  it('round-trips an encoded cursor', () => {
    const cursor = { createdAt: '2026-07-26T08:30:00.000Z', id: randomUUID() };
    const encoded = encodeProcurementCursor(cursor);
    expect(encoded).not.toContain('{');
    expect(decodeProcurementCursor(encoded)).toEqual(cursor);
  });

  it('returns null for an absent cursor', () => {
    expect(decodeProcurementCursor(undefined)).toBeNull();
    expect(decodeProcurementCursor('')).toBeNull();
  });
  ...
  it.each([
    ...
    ['invalid uuid', Buffer.from(JSON.stringify({
      createdAt: '2026-07-26T08:30:00.000Z', id: 'supplier-1'
    })).toString('base64url')]
  ])('rejects %s with INVALID_CURSOR', (_label, value) => {
    let caught: unknown;
    try {
      decodeProcurementCursor(value);
    } catch (error) {
      caught = error;
    }
